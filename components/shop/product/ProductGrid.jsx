...
const ProductGrid = ({ items, handleListItemClick, title, className }) => {
    return (
        <div className={`m-4 xxs:max-h-[calc(100vh-150px)] md:max-h-[calc(100vh-100px)] overflow-y-auto overscroll-y-none ${className}`}>
            <h2>{title}</h2>
            <div className={"grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4"}>
                {items.map((product) => (
                    <div key={product.id} className="flex flex-col p-4 border border-bg-gray-50 bg-white items-center rounded-[4px] cursor-pointer" onClick={() => handleListItemClick(product.id)}>
