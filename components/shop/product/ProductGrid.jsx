...
const ProductGrid = ({ items, handleListItemClick, title, className }) => {
    return (
        <div className={`xxs:max-h-[calc(100vh-150px)] md:max-h-[calc(100vh-100px)] ${className}`}>
            <h2>{title}</h2>
            <div className={"grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4"}>
                {items.map((product) => (
                    <div key={product.id} className="flex flex-col bg-white items-start rounded-[4px] cursor-pointer" onClick={() => handleListItemClick(product.id)}>
